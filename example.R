...
#creating MVPA variable
plotdata <- plotdata %>% mutate(dur_day_total_MVPA = dur_day_total_MOD_min_pla + dur_day_total_VIG_min_pla)

#join demographic, pubertal status, impairment, BMI, and ethnicity data to activity/actigraphy data
...
#rescoring peterson pubertal scale
plotdata <- plotdata %>%
...
#Looking at missing data and subsetting accordingly now 
#here taking away data that is missing Sex data, for example

missing_demo <- plotdata %>% filter(is.na(Sex))
plotdata <- filter(plotdata, ! ID %in% missing_demo$ID)

#Exploratory analyses & renaming more variables (have not included all exploratory here)
